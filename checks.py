...
def last_update(row, current_time) -> (str, str):
...
def positives_rate(row, current_time) -> (str, str):
    """Check that positives compose <20% test results"""

    error = None
    warning = None

    n_pos, n_neg, n_deaths = row.Positive, row.Negative, row.Deaths
...
def death_rate(row, current_time) -> (str, str):
...
def less_recovered_than_positive(row, current_time) -> (str, str):
...
def pendings_rate(row, current_time) -> (str, str):
    """Check that pendings are not more than 20% of total"""

    error = None
    warning = None

    n_pos, n_neg, n_pending, n_deaths = row.Positive, row.Negative, row.Pending, row.Deaths
    n_tot = n_pos + n_neg + n_deaths
